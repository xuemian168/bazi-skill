# bazi-skill: 八字排盘、紫微斗数、星座合参、合盘与择日择时 Skill
...
`bazi-skill` 是一套可复用的 Codex / Claude Code Skill，用于八字排盘、四柱命理、真太阳时、可选紫微斗数事实、可选西洋占星/星座事实、运势 K-line JSON、合盘合婚、择日择时和专业命理报告工作流。紫微和星座/占星事实只有在代码已计算或用户提供确认盘时才能使用；当前不提供 PDF 导出能力。

关键词：八字排盘、四柱、紫微斗数、星座、西洋占星、真太阳时、合盘、合婚、择日、择时、吉日吉时、Codex Skill、Claude Code Skill、BaZi、Zi Wei Dou Shu、Western astrology、zodiac signs、natal chart、synastry、Chinese astrology、true solar time。
...
- 代码排盘，AI 只解读：八字四柱、大运、流年、紫微盘、西洋占星星盘/星座/相位、合盘关系、吉时干支等事实必须由宿主项目代码或确定性脚本计算，不能由 AI 口算或重算。
...
- 八字、紫微与星座流程：八字默认围绕 `lunar-javascript` 或宿主项目已有确定性实现；紫微和西洋占星必须先确认 repo 中已有确定性实现或用户已提供确认盘，不能假设存在 `ziweiService.ts` 或 astrology service。
- 真太阳时处理：区分宿主项目的经度修正口径与更严格的视太阳时口径，并处理时辰边界风险。
- 运势 K-line JSON：生成、修复、审查符合宿主项目 `AnalysisResult` 合约的 100 年时间线数据。
- 多流派大师会诊：支持子平格局、旺衰扶抑、调候、盲派象法、神煞辅助、紫微、西洋占星、择日、合盘等角色分工，最终由裁判综合。
...
### 案例 2：多流派大师会诊（裁判综合，节选）

对上盘触发「多流派大师会诊」：并行派发 **子平格局 / 旺衰扶抑 / 调候 / 盲派象法 / 神煞辅助** 五派，各派只拿同一份证据包 + 自家流派提示词，禁止重算命盘；再经 **安全编辑** 把关，最后由 **裁判** 按源层级（代码事实 > 契约 > 方法契合度 > 跨派共识 > 单派解读）综合，不机械平均。
...
承案例 1–2 的同一随机样本，触发「聚焦某一维度深推」。裁判先用 `lunar-javascript` 确定性算出流年干支，并标注其与**夫星（女命官杀=土）、夫宫（日支辰）、桃花（时支子）**的合冲关系，再据此综合——全程为**结构倾向 / 时机参考，非"克夫/注定"断语**。
...
- `references/`：分主题参考文档，包括项目合约、分析方法、真太阳时、紫微、星座/西洋占星、合盘、择时、报告生成、角色分工等。
...
在宿主项目根目录启动 Claude Code 后，可以用 `/bazi-skill` 调用同一套八字、可选紫微事实、可选星座/西洋占星事实、合盘、择日择时、多流派大师会诊和专业报告工作流。该镜像使用项目本地 `${CLAUDE_SKILL_DIR}` 读取 `references/` 与 `scripts/`，避免依赖个人机器上的 Codex skill 安装路径。
