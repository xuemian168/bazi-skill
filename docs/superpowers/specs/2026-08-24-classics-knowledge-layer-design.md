...
这条覆盖关系**由裁判与报告作者执行，不由脚本检查**（见计划偏差 E）：正文不带
角标意味着合规报告的正文里按定义不出现卡片 ID，脚本无从把「结构性论断」与
索引行对上 —— 与「孤证不立」的「事件级判断」同类，无法从自由文本可靠分类。
`validate_citations.py --answer` 只校验可机械判定的部分：正文若确实出现了卡片
ID，该 ID 必须在索引中；索引中不得出现不存在的卡片；索引若一条卡片都没列出，
报告必须显式写 `citations:` 或在相应段落写明「无典籍条文支撑」。
...
5. 「依据索引」章节存在；正文中出现的卡片 ID 都在该章节内；该章节不列出不存在的
   卡片；该章节若一条卡片都没列出，报告须显式写 `citations:` 或写明
   「无典籍条文支撑」。**「覆盖正文全部结构性论断」不做脚本检查**，理由见 §8.3
   与计划偏差 E。
